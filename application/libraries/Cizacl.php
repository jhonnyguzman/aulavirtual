...
class CI_Cizacl
{
	...
	function __construct()
	{
		...
		foreach($qRoles->result() as $role)	{
			if(!empty($role->inherit_id))	{
				foreach(json_decode($role->inherit_id) as $role_inherit)	{
					$role_inherit_array[] = $role_inherit;
				}
			}
			if(!empty($role->inherit_id))
				$this->cizacl->addRole(new Zend_Acl_Role($role->id),$role_inherit_array);
			else
				$this->cizacl->addRole(new Zend_Acl_Role($role->id));
		}
		...
		foreach($qRules->result() as $rules)	{
			...
			$resources = array();
			foreach(json_decode($rules->resource_controller) as $resource)	{
				if(!empty($resource))
					$resources[] = $resource;
			}
			if($rules->cizacl_rule_type == 'allow')	{
				...
				$this->cizacl->deny($rules->role_id,$resources,$privileges);
		}
	...
	function check_cizacl()
	{
		...
		if($this->cizacl_mdl->getDefaultUser($this->session->userdata('role_id')))
			$role = $this->cizacl_mdl->getDefaultUser($this->session->userdata('role_id'));
		else
	...
	function check_hasRole($role)
	{
		if(is_numeric($role))
			...
			$this->db->where('LOWER(name)',strtolower($role));
			$query = $this->db->get('roles');
			if($query->num_rows())	{
				$row = $query->row();
				return $this->cizacl->hasRole($row->id);
			}
			return false;
		}
	}
