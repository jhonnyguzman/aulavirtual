...
class Cizacl_Js extends CI_Controller {
		...
		$(document).ready(function()	{
			$("'.$jqgrid_name.'").jqGrid({
				...
				colModel :[ 
					...
					{name:"lastaccess", index:"lastaccess", align:"center"},
					{name:"added_by", index:"added_by", align:"center"},
					{name:"added_by_id", index:"added_by_id", hidden: true}
				],
				autowidth: true,
				height: "auto",
				setGridHeight: "100%",
					...
					if(ui.index == 0 && !jqgrid_initialized[0]){
						$("'.$jqgrid_name[0].'").jqGrid({
							...
							colModel :[ 
								{name:"id", index:"id", hidden:true}, 
								{name:"name", index:"name"},
								{name:"inherit_id", index:"inherit_id", align:"center"},
								{name:"redirect", index:"redirect", align:"center"},
								{name:"description", index:"description"},
								{name:"order", index:"order", align:"center"},
								{name:"default", index:"default", align:"center"}
							],
							autowidth: true,
							height: "auto",
							setGridHeight: "100%",
	...
	public function rule_oper()
	{
		...
		function getRules()	{
			$.ajax({
				url:		"'.site_url("cizacl/getRules").'/"+$("#role_id").val(),
				type:		"post",
				dataType:	"json",
				success:	function(data)	{
					...
					if(data.response == "success")	{
						for(i = 0; i < (data.rows).length; i++)	{
							controllers	= $.parseJSON(data.rows[i]["resource_controller"]).toString();
							controllers	= controllers.split(/,/);
							functions	= $.parseJSON(data.rows[i]["resource_function"]).toString();
							functions	= functions.split(/,/);
							addRule(data.rows[i]["type"],controllers[0],functions[0],data.rows[i]["status"],data.rows[i]["description"]);
